...
// ###%%%###%%%###%%%###%%%###%%%###%%%###%%%###%%%###%%%###%%%###%%%###%%%
function addToView(targetID, doc) {
    const clearing = false;
    return spliceIntoPage(targetID, doc, clearing);
}
function refreshView(targetID, doc) {
    const clearing = true;
    return spliceIntoPage(targetID, doc, clearing);
}
const clearElementById = (targetID) => {
    const clearMe = document.getElementById(targetID);
    // clears everything inside by removing until none are left
    if (clearMe) {
        clearElement(clearMe);
    }
    else {
        console.log("Can't clear " + targetID);
    }
};
...
function spliceIntoPage(targetID, doc, clearing) {
    const viewerElement = document.getElementById(targetID);
    if (viewerElement && doc.documentElement) {
        const newContents = document.adoptNode(doc.documentElement);
        if (clearing) {
            clearElement(viewerElement);
        }
        ;
        ...
        if (!viewerElement) {
            console.log(" ... not seeing " + targetID);
        }
    }
}
/*
...
function transformAndShowXML(xmlLiteral, xsltHREF, targetID) {
    return __awaiter(this, void 0, void 0, function* () {
        const resultDocument = yield getAndApplyXSLTtoLiteral(xmlLiteral, xsltHREF); // a promise
        const clearing = true;
...
function getAndApplyXSLTtoLiteral(xmlLiteral, xsltHREF) {
    return __awaiter(this, void 0, void 0, function* () {
        const xsltEngine = yield xsltEngineForHREF(xsltHREF);
        return parseAndTransformXMLLiteral(xmlLiteral, xsltEngine);
    });
}
function getAndApplyXSLTtoXMLatHREF(xmlHREF, xsltHREF) {
    return __awaiter(this, void 0, void 0, function* () {
        const xsltEngine = yield xsltEngineForHREF(xsltHREF);
...
function xsltEngineForHREF(xsltHREF) {
    return __awaiter(this, void 0, void 0, function* () {
        const xsltDoc = yield getXMLviaHTTP(xsltHREF);
        return setupXSLTEngine(xsltDoc);
    });
}
// Setting up the engine - extend to provide runtime parameters (in a map)
function setupXSLTEngine(xslt) {
...
function parseAndTransformXMLatHREF(httpResource, xsltEngine) {
    return __awaiter(this, void 0, void 0, function* () {
        const doc = yield getXMLviaHTTP(httpResource);
        // error handling?
        return xslt1ResultDocument(doc, xsltEngine);
    });
}
// If we have an XML DOM and a configured XSLT engine ... returning a Document
function xslt1ResultDocument(xmlDom, xsltEngine) {
    // error handling?
...
function xslt1ResultFragment(xmlDom, xsltEngine) {
    // error handling?
...
function getXMLviaHTTP(httpResource) {
    return new Promise((resolve, reject) => {
...
function JSONasXML(json) {
    try {
        JSON.parse(json, obj => writeOBJtoXML(obj));
    }
    catch (error) {
...
function writeOBJtoXML(obj) {
    let xml = "";
    for (const prop in obj) {
        xml += tagProperty(obj[prop], prop, true);
    }
    ;
    return xml;
}
// The tagger returns tagging for anything, including its contents
function tagProperty(obj, key, top) {
    ...
    if (scalarTypes.includes(typeof obj)) {
        xml += obj;
    }
    // or for objects and arrays, we process contents recursively
    else if (typeof obj === "object") {
        for (const prop in obj) {
            xml += tagProperty(obj[prop], prop, false);
        }
    }
    ;
    // now the close tag
...
function fileTextContent(fileToRead) {
    return new Promise((resolve, reject) => {
...
// ###%%%###%%%###%%%###%%%###%%%###%%%###%%%###%%%###%%%###%%%###%%%###%%%
function addClassToElementById(who, newClass) {
    const whichElement = document.getElementById(who);
    if (whichElement) {
        whichElement.classList.add(newClass);
    }
}
function removeClassFromElementById(who, theClass) {
    const whichElement = document.getElementById(who);
    if (whichElement) {
        whichElement.classList.remove(theClass);
    }
}
function addClassToElementsByClass(addToClass, newClass) {
    const elements = document.getElementsByClassName(addToClass);
    for (const elem of elements) {
        elem.classList.add(newClass);
    }
}
function removeClassFromElementsByClass(removeFromClass, theClass) {
    const elements = document.getElementsByClassName(removeFromClass);
    for (const elem of elements) {
...
//# sourceMappingURL=xslt-blender.js.map