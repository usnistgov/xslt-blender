...
describe('When rendering JSON as XML ...', () => {
    ...
    it('JSON to XML function must deliver XPath JSON XML (simple)', () => {
        ...
        const aJSONobj:object = { someProperty: "string value" } ;
        
        const testXML: string = writeOBJtoXML(aJSONobj);
        const testXMLdoc: Document = parseXMLLiteral(testXML);
        const intendedXML: string = '<map xmlns="http://www.w3.org/2005/xpath-functions"><string key="someProperty">string value</string></map>'
        const intendedXMLdoc: Document = parseXMLLiteral(intendedXML);
        assert.equal(testXMLdoc, intendedXMLdoc);
    });
    it('JSON to XML function must deliver XPath JSON XML (type sampling)', () => {

        const aJSONobj:object = { aMap: { str1: "x", str2: "y"},
                                  anArray: [ 1, 2, 3],
                                  aBoolean: false,
                                  aNumber: 1.62,
                                  aString: "A STRING" };
